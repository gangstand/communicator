...
if TYPE_CHECKING:
    ...
    from app.schemas.auth import AuthUserLogin
    from domain import BaseRepository
    from domain.typing import OptionalBoolean, OptionalString


class EntityAuth:
    token: OptionalString = None
    token_location: ClassVar[set] = {"headers"}

    secret_key: OptionalString
    public_key: OptionalString = None
    private_key: OptionalString = None
    algorithm: str = "HS256"
    decode_algorithms: set | None = None
    decode_leeway: timedelta | str = 0
    encode_issuer: OptionalString = None
    decode_issuer: OptionalString = None
    decode_audience: OptionalString = None
    deny_list_enabled: OptionalBoolean = False
    deny_list_token_checks: ClassVar[set] = {"access", "refresh"}
    header_name: OptionalString = "Authorization"
    header_type: OptionalString = "Bearer"
    ...
    access_cookie_key: OptionalString = "access_token_cookie"
    refresh_cookie_key: OptionalString = "refresh_token_cookie"
    access_cookie_path: OptionalString = "/"
    refresh_cookie_path: OptionalString = "/"
    cookie_max_age: OptionalString = None
    cookie_domain: OptionalString = None
    cookie_secure: OptionalBoolean = False
    cookie_same_site: OptionalString = None

    # option for double submit csrf protection
    cookie_csrf_protect: OptionalBoolean = True
    access_csrf_cookie_key: OptionalString = "csrf_access_token"
    refresh_csrf_cookie_key: OptionalString = "csrf_refresh_token"
    access_csrf_cookie_path: OptionalString = "/"
    refresh_csrf_cookie_path: OptionalString = "/"
    access_csrf_header_name: OptionalString = "X-CSRF-Token"
    refresh_csrf_header_name: OptionalString = "X-CSRF-Token"
    csrf_methods: ClassVar[set] = {"POST", "PUT", "PATCH", "DELETE"}
    ...
    @classmethod
    def token_in_deny_list_loader(cls, callback: Callable[..., bool]) -> None:
    ...
    def _get_secret_key(self, algorithm: str, process: str) -> OptionalString:
    ...
    def _create_token(
            self, subject: str | int, type_token: str, exp_time: int | None, fresh: OptionalBoolean = None,
            algorithm: OptionalString = None, headers: dict | None = None, issuer: OptionalString = None,
            audience: OptionalString = None, user_claims: dict | None = None,
    ) -> str:
        access_type = "access"
    ...
    def _get_expired_time(
            self,
            type_token: str,
            expires_time: timedelta | int | OptionalBoolean = None,
    ) -> int | None:
        access_type = "access"
    ...
    def create_access_token(
            self,
            subject: str | int,
            fresh: OptionalBoolean = None,
            algorithm: OptionalString = None,
            headers: dict | None = None,
            expires_time: timedelta | int | OptionalString = None,
            audience: OptionalString = None,
    ...
    def _verify_and_get_jwt_optional_in_cookies(
            ...
            csrf_token: OptionalString = None,
    ) -> None:
    ...
    def _verifying_token(self, encoded_token: str, issuer: OptionalString = None) -> None:
    ...
    def _verified_token(self, encoded_token: str, issuer: OptionalString = None) -> dict:
    ...
    def get_raw_jwt(self, encoded_token: OptionalString = None) -> dict | None:
    ...
    def get_jwt_subject(self) -> str | int | None | uuid.UUID:
        if self.token:
            return self._verified_token(self.token)["sub"]
        return None

    def get_unverified_jwt_headers(self, encoded_token: OptionalString = None) -> dict:
        return jwt.get_unverified_header(encoded_token or self.token)

    @staticmethod
    async def check_password(user: UserRead, data: AuthUserLogin) -> UserRead:
    ...
    @staticmethod
    async def verify(auth: EntityAuth) -> None:
        try:
            auth.jwt_required()
    ...
    @staticmethod
    async def verify_user(_repository: BaseRepository, **kwargs: dict[str, Any] | str | uuid.UUID) -> UserRead:
        try:
            user = await _repository.query_entity(**kwargs)
        except HTTPException as e:
            raise HTTPException(status_code=401, detail="Not Found User") from e
        return user

    async def verify_token_password(self, token: str) -> uuid.UUID:
        try:
            entity_id = jwt.decode(token, self.secret_key, algorithms=["HS256"]).get("sub")
        except jwt.ExpiredSignatureError as e:
    ...
    async def create_token_password(
            self, entity_id: uuid.UUID, expires_delta: timedelta = timedelta(minutes=15),
    ) -> str:
        return jwt.encode(
            {"sub": str(entity_id), "exp": datetime.now(timezone.utc) + expires_delta}, self.secret_key,
            algorithm="HS256",
        )
